...
def entail_check(prediction, golden_answers):
    if isinstance(golden_answers, str):
        golden_answers = [golden_answers]
    normalized_prediction = normalize_answer(prediction)
    score = 0
...
def f1_check(prediction, golden_answers):
    ...
    for golden_answer in golden_answers:
        ...
        if normalized_prediction in ["yes", "no", "noanswer"] and normalized_prediction != normalized_golden_answer:
            continue

        prediction_tokens = normalized_prediction.split()
        golden_answer_tokens = normalized_golden_answer.split()
...
def compute_entail(
    ...
    if answer is None:
        ...
        if entail_check(answer, ground_truth["target"]):
            return True
        else:
            return False
