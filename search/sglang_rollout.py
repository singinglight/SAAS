...
from search.rm_hub import batched_async_rm_search, async_eval_rm_search
from search.utils import get_metrics, get_metrics_ggpo
...
class GenerateState(metaclass=SingletonMeta):
    ...
    def submit_generate_tasks(self, samples: list[list[Sample]], step) -> None:
    ...
    async with state.semaphore:
        ...
        with state.dp_rank_context() as _:
            ...
            if custom_func_path is not None:
                ...
                # Check if custom function accepts group_idx parameter
                if "group_idx" in inspect.signature(custom_generate_func).parameters:
                    sample = await custom_generate_func(args, sample, sampling_params, group_idx=group_idx)
                elif "evaluation" in inspect.signature(custom_generate_func).parameters:
...
async def eval_generate_and_rm(
    args: Namespace,
    sample: Sample,
    sampling_params: dict[str, Any],
    evaluation: bool = False,
    ...
    if sample.status == Sample.Status.COMPLETED or sample.status == Sample.Status.TRUNCATED:
        assert sample.response is not None
        reward, result = await async_eval_rm_search(args, sample)
        sample.reward = reward
...
async def generate_and_rm_group(
    args: Namespace, group: list[Sample], sampling_params: dict[str, Any], evaluation: bool = False, step: int = 0
) -> list[Sample]:
    state = GenerateState(args)
    ...
    for idx, sample in enumerate(group):
        ...
        tasks.append(
            asyncio.create_task(generate_and_rm(args, sample, current_sampling_params, evaluation=evaluation, group_idx=idx))
        )
    ...
    if not state.aborted and args.group_rm:
        rewards, line_obj = await batched_async_rm_search(args, group)
        for sample, reward in zip(group, rewards, strict=False):
            sample.reward = reward

        if evaluation:
            reward_info_dir=f'{args.reward_info_path}/valdata'
            os.makedirs(reward_info_dir, exist_ok=True)
            reward_info_file=f'{reward_info_dir}/step_{str(step).zfill(3)}_valStep_{str(args.eval_interval).zfill(3)}.jsonl'
        else:
            reward_info_dir=f'{args.reward_info_path}/expdata'
            os.makedirs(reward_info_dir, exist_ok=True)
            reward_info_file=f'{reward_info_dir}/step_{str(step).zfill(3)}.jsonl'
        with open(reward_info_file, "a", encoding="utf-8") as f:
...
async def generate_rollout_async(
    ...
    while len(data) < target_data_size:
        while state.remaining_batch_size < target_data_size:
            ...
            state.submit_generate_tasks(samples, step)
    ...
    if args.disable_search_tokens_for_first_half:
        sample_metrics = get_metrics_ggpo(args, all_samples, False)
    else:
        sample_metrics = get_metrics(args, all_samples, False)
...
async def eval_rollout(args: Namespace, rollout_id: int) -> tuple[dict[str, dict[str, list[Any]]], list[list[Sample]]]:
    ...
    for ds_result in results.values():
        all_samples.extend(ds_result["samples"])
    metrics = get_metrics(args, all_samples, True)
...
async def eval_rollout_single_dataset(
    ...
    final_results.sort(key=lambda result: result["id"])

    step = compute_rollout_step(args, rollout_id)
    reward_info_dir=f'{args.reward_info_path}/valdata'
    os.makedirs(reward_info_dir, exist_ok=True)
    reward_info_file=f'{reward_info_dir}/step_{str(step).zfill(3)}_valStep_{str(args.eval_interval).zfill(3)}_single.jsonl'
