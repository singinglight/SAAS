...
from search.utils import calculate_score_smooth_label_V1, calc_repetition_rate, calculate_linear_score_small, calculate_linear_score_large
...
async def generate(args, sample: Sample, sampling_params, group_idx: int = None) -> Sample:
    ...
    for _turn_idx in range(SEARCH_R1_CONFIGS["max_turns"]):
        ...
        next_obs, done, search_query, title_to_doc, title_to_doc_id = await execute_predictions(cur_response)
        if done:
            ...
            if doc_count > 0:
                ...
                if lap_ratio >= args.lap_ratio_threshold:
                    is_doc_rep = True

            sample.is_doc_rep = is_doc_rep
...
async def mq_group_reward_func(args, samples, **kwargs):
    ...
    exist_em_tail = False
    if args.linear_fn:
        if args.use_small_slope:
            score_func = calculate_linear_score_small
        else:
            score_func = calculate_linear_score_large
    else:
        score_func = calculate_score_smooth_label_V1
    ...
    for sample in samples:
        ...
        if args.doc_rep_penalty and sample.is_doc_rep:
            reward = reward - 0.1 * sample.lap_ratio
        ...
        if args.format_check and not sample.is_format:
            reward = reward - 0.2

        if args.repetition_detect and sample.is_rsp_rep:
            reward = reward - 0.5

        if args.retrieval_penalty:
            if sample.is_retrieval:
                if not args.retrieval_em_different:
                    reward = reward + 0.1
                else:
    ...
    line_obj = {
        ...
        "group_format_check": [int(x["format_check"]) for x in group_result],
        ...
        "group_doc_rep": [x["is_doc_rep"] for x in group_result],
