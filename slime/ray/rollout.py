...
class RolloutManager:
    ...
    #     exact_match = [sample.exact_match for sample in samples]
    #     is_nq = [sample.metadata['data_source'] == "train_searchR1_nq" for sample in samples]
    #     if (
    ...
    #         exact_match = torch.tensor(exact_match, dtype=torch.bool)
    #         is_nq = torch.tensor(is_nq, dtype=torch.bool)
    #         if rewards.shape[-1] == self.args.n_samples_per_prompt * self.args.rollout_batch_size:
    #             rewards = rewards.reshape(-1, self.args.n_samples_per_prompt)
    #             exact_match = exact_match.reshape(-1, self.args.n_samples_per_prompt)
    #             is_nq = is_nq.reshape(-1, self.args.n_samples_per_prompt)
    #         else:
    #             # when samples count are not equal in each group
    #             rewards = rewards.view(-1, rewards.shape[-1])
    #             exact_match = exact_match.view(-1, exact_match.shape[-1])
    #             is_nq = is_nq.view(-1, is_nq.shape[-1])

    #         # For hotpotqa groups, normalize the entire group; otherwise use half-group normalization
    #         first_half_mask = ~is_nq
    #         n_samples_per_prompt = self.args.n_samples_per_prompt // 2
    #         first_half_rewards = rewards[:, :n_samples_per_prompt]
    #         second_half_rewards = rewards[:, n_samples_per_prompt:]

    #         # Normalize first half
    #         mean = first_half_rewards.mean(dim=-1, keepdim=True)
    #         first_half_rewards = first_half_rewards - mean
    #         std = first_half_rewards.std(dim=-1, keepdim=True)
    #         first_half_rewards = first_half_rewards / (std + 1e-6)

    #         # Normalize second half
    #         mean = second_half_rewards.mean(dim=-1, keepdim=True)
    #         second_half_rewards = second_half_rewards - mean
    #         std = second_half_rewards.std(dim=-1, keepdim=True)
    #         second_half_rewards = second_half_rewards / (std + 1e-6)

    #         # For nq groups, replace both halves with full-group normalization
    #         nq_mask = is_nq[:, 0]  # all elements in a group share the same data_source
    #         if nq_mask.any():
    #             full_rewards = rewards.clone()
    #             for i in range(full_rewards.shape[0]):
    #                 if nq_mask[i]:
    #                     mean = full_rewards[i].mean()
    #                     full_rewards[i] = (full_rewards[i] - mean) / (full_rewards[i].std() + 1e-6)
    #             first_half_mask_h = first_half_mask[:, :n_samples_per_prompt]
    #             second_half_mask_h = first_half_mask[:, n_samples_per_prompt:]
    #             first_half_rewards = torch.where(first_half_mask_h, first_half_rewards, full_rewards[:, :n_samples_per_prompt])
    #             second_half_rewards = torch.where(second_half_mask_h, second_half_rewards, full_rewards[:, n_samples_per_prompt:])
    ...
    #         # if n_samples > 1:
    #         #     half_n = n_samples // 2
    #         #     mean_first_half = exact_match[:, :half_n].float().mean(dim=-1, keepdim=True)
    #         #     mean_second_half = exact_match[:, half_n:].float().mean(dim=-1, keepdim=True)
    #         #     first_better = mean_first_half > mean_second_half
    #         #     multipliers = torch.ones_like(rewards)
    #         #     multipliers[:, :half_n] = torch.where(first_better, 2, 1.0)
    #         #     multipliers[:, half_n:] = torch.where(first_better, 1.0, 2)
    ...
    def _post_process_rewards(self, samples: list[Sample] | list[list[Sample]]):
        ...
        raw_rewards = [sample.get_reward_value(self.args) for sample in samples]
        exact_match = [sample.exact_match for sample in samples]
        is_hotpotqa = [sample.metadata['data_source'] == "train_searchR1_hotpotqa" for sample in samples]
        if (
            self.args.advantage_estimator in ["grpo", "gspo", "reinforce_plus_plus_baseline"]
            and self.args.rewards_normalization
        ):
            ...
            rewards = torch.tensor(raw_rewards, dtype=torch.float)
            exact_match = torch.tensor(exact_match, dtype=torch.bool)
            is_hotpotqa = torch.tensor(is_hotpotqa, dtype=torch.bool)
            if rewards.shape[-1] == self.args.n_samples_per_prompt * self.args.rollout_batch_size:
                rewards = rewards.reshape(-1, self.args.n_samples_per_prompt)
                exact_match = exact_match.reshape(-1, self.args.n_samples_per_prompt)
                is_hotpotqa = is_hotpotqa.reshape(-1, self.args.n_samples_per_prompt)
            else:
                # when samples count are not equal in each group
                rewards = rewards.view(-1, rewards.shape[-1])
                exact_match = exact_match.view(-1, exact_match.shape[-1])
                is_hotpotqa = is_hotpotqa.view(-1, is_hotpotqa.shape[-1])

            # For hotpotqa groups, normalize the entire group; otherwise use half-group normalization
            first_half_mask = ~is_hotpotqa
            n_samples_per_prompt = self.args.n_samples_per_prompt // 2
            first_half_rewards = rewards[:, :n_samples_per_prompt]
            second_half_rewards = rewards[:, n_samples_per_prompt:]
            ...
            # For hotpotqa groups, replace both halves with full-group normalization
            hotpotqa_mask = is_hotpotqa[:, 0]  # all elements in a group share the same data_source
            if hotpotqa_mask.any():
                full_rewards = rewards.clone()
                for i in range(full_rewards.shape[0]):
                    if hotpotqa_mask[i]:
                        mean = full_rewards[i].mean()
